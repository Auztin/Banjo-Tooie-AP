  ...
  switch (type) {
    ...
    case AP_ITEM_GRRELIC:
      if (value != totals->green_relics) {
        ...
        if(totals->green_relics >= ap_memory.pc.settings.green_relics_chamber_requirement && totals->green_relics <= ap_memory.pc.settings.green_relics_boss_requirement && bt_current_map == BT_MAP_MT_TARGITZANS_TEMPLE)
          bt_fn_object_anim(0x2b4, 7, 0);
        if(totals->green_relics >= ap_memory.pc.settings.green_relics_boss_requirement && bt_current_map == BT_MAP_MT_TARGITZANS_TEMPLE)
          bt_fn_object_anim(0x2b4, 7, 1);
