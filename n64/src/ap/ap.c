			...
			switch (ap.input.cmd) {
				...
				case APC_LOCATIONS:
					for (u32 i = 0; i < (ap.input.size-2)/sizeof(*ap.input.locations); i++) {
						...
						set_bit(ap.in.locations, location);
					}
					break;
				case APC_ITEMS:
...
void ap_output() {
	...
	for (int i = 0; i < APL_MAX_BYTES; i++) {
		u8 location = ap.locations[i];
		u8 sent_location = ap.out.locations[i];
		if (location == sent_location) continue;
		u32 base = i*8;
		for (int b = 0; b < 8; b++) {
			u32 id = base+b;
			if (!get_bit(ap.locations, id) || set_bit(ap.out.locations, id)) continue;
